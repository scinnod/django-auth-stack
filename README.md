...
This interactive script handles both **initial setup** and **reconfiguration**:
- Generates secure secrets (or preserves existing ones)
- Configures domain and Keycloak settings
- Sets up admin console security (rate limiting, optional IP restriction)
- **Interactively configures upstream services** (add, edit, enable/disable)
- Creates a clean `.env` file (backs up existing configuration)

**Reconfiguration:** Run the same script again to modify settings. Existing values are shown as defaults, and secrets are preserved unless you explicitly regenerate them.
...
> **Note:** `configure-env.sh` runs this automatically at the end.
> You only need to run it manually if you edit `.env` by hand.
...
```
This generates certificates AND DH parameters in `certs/`, sets `LETSENCRYPT_ENABLED=false`,
and regenerates `docker-compose.override.yml` (without certbot).
...
```bash
# 1. Set LETSENCRYPT_ENABLED=true in .env (or use configure-env.sh)
# 2. Run the init script (auto-regenerates override with certbot)
...
To change any settings (domains, services, security options), run the configuration script:

```bash
./scripts/configure-env.sh
```

The script detects your existing `.env` and:
- Uses current values as defaults (just press Enter to keep)
- Preserves secrets unless you explicitly type `YES` to regenerate
- Allows editing, disabling, or removing services
...
```bash
./scripts/configure-env.sh
# Follow prompts to add new services or edit/remove existing ones
# (override is regenerated automatically at the end)
docker network create <new_service>_backend
...
1. Enable Let's Encrypt in `.env`:
   ```bash
   LETSENCRYPT_ENABLED=true
   LETSENCRYPT_EMAIL=your-email@example.com
   ```
   Or use `configure-env.sh`, which prompts for this.
...
2. Run init script:
   ...
   The script will:
   - Verify `LETSENCRYPT_ENABLED=true` is set (exits with instructions if not)
   - Auto-regenerate `docker-compose.override.yml` with certbot (if needed)
   ...
   - Request real certificates from Let's Encrypt (with debug output)
...
3. Start the stack (override is auto-loaded):
   ```bash
   docker compose up -d
   ```
...
Service networks are added via `docker-compose.override.yml`, which is generated
from your `.env` configuration. It is auto-generated by `configure-env.sh`,
`init-selfsigned.sh`, and `init-letsencrypt.sh`. Run it manually only if you
edit `.env` by hand:
...
2. **Use real TLS certificates**:
   ...
   - Or enable Let's Encrypt: set `LETSENCRYPT_ENABLED=true` in `.env` and run `./scripts/init-letsencrypt.sh`
...
4. **Secure all passwords** in `.env` (auto-generated by configure-env.sh script)
5. **Configure service networks**: Create Docker networks for each service
6. **Generate compose override**: Run `./scripts/generate-compose-override.sh` (or let `configure-env.sh` do it)
...
1. ✅ Configure services in `.env` (via `configure-env.sh` or manually)
2. ✅ Override is auto-generated (or run `./scripts/generate-compose-override.sh`)
